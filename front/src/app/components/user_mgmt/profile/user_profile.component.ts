...
export class ProfileComponent implements OnInit {
  profileImageUrl: string =
    "front/src/assets/images/user_profiles/download.jpeg";
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files[0]) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        ...
        Swal.fire(
          "Error",
          "El archivo es demasiado grande. Tamaño máximo: 5 MB.",
          "error"
        );
        return;
      }
  ...
  calcularPromedioCalificacion(): number {
    ...
    const sumaCalificaciones = this.resenias.reduce(
      (sum, resenia) => sum + resenia.calificacion,
      0
    );
