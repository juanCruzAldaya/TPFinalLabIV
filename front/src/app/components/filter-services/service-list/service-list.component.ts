...
export class ServiceListComponent implements OnInit {
  ...
  services: IServicio[] = []; // Array of Servicio
  showCalendar: boolean = false;
  filteredServices: IServicio[] = []; // Array of Servicio filtered
  ...
  loadAdditionalData(): void {
    this.services.forEach((service) => {
      this.filterService
        .getCategoria(service.sub_categoria)
        .subscribe((categoria) => {
      ...
      .filter(
        (service) =>
          (this.filterCriteria.localidad
            ? service.localidad.includes(this.filterCriteria.localidad)
            : true) &&
          (this.filterCriteria.nombre
            ? service.nombre.includes(this.filterCriteria.nombre)
            : true) &&
          (this.filterCriteria.calificacion
            ? service.calificacion === +this.filterCriteria.calificacion
            : true) &&
          (this.filterCriteria.categoria
            ? service.categoria.includes(this.filterCriteria.categoria)
            : true) &&
          (this.filterCriteria.subCategoria
            ? service.subCategoria.includes(this.filterCriteria.subCategoria)
            : true) &&
          (this.filterCriteria.nombreProfesional
            ? service.nombreProfesional.includes(
                this.filterCriteria.nombreProfesional
              )
            : true)
      );
  ...
  handleDayClick(event: { day: CalendarMonthViewDay<any>; date: string }) {
    ...
    this.calendarService
      .getAvailableSlots(this.selectedService.profesional_id, this.selectedDate)
      .subscribe(
        (availableSlots: string[]) => {
  ...
  onDayClicked(date: string) {
    ...
    this.calendarService
      .getAvailableSlots(this.selectedService.profesional_id, date)
      .subscribe(
        (slots: any[]) => {
