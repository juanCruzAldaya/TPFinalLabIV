...
export class ServiceListComponent implements OnInit {
  ...
  selectedService: any;
  profesional_id: number | null = null;
  isModalOpen = false;
  filterCriteria = {
    localidad: "",
    nombre: "",
  ...
  loadServices(): void {
    ...
    const newServices = this.serviceCards
      .slice(start, end)
      .filter(
        (serviceCard) =>
          ...
          (this.filterCriteria.categoria
            ? serviceCard.category_name.includes(this.filterCriteria.categoria)
            : true) &&
          (this.filterCriteria.subCategoria
            ? serviceCard.subCategory_name.includes(
                this.filterCriteria.subCategoria
              )
            : true) &&
          (this.filterCriteria.nombreProfesional
            ? serviceCard.profesional_name.includes(
                this.filterCriteria.nombreProfesional
              )
            : true)
      );
  ...
  handleDayClick(event: { day: CalendarMonthViewDay<any>; date: string }) {
    ...
    this.calendarService
      .getAvailableSlots(this.profesional_id!, this.selectedDate)
      .subscribe(
        (availableSlots: string[]) => {
